...
class Dataset(data.Dataset):
    ...
    def load_labels(filenames):
        ...
        for filename in filenames:
            ...
            fl_nm = filename.replace('images', 'matlab_files')
            bs_nm = 'poly_gt_'+os.path.basename(fl_nm).replace('.jpg','.json')
            gt_file = os.path.join(os.path.dirname(fl_nm),bs_nm)
            with open(gt_file, 'r') as f:
                jfl = json.load(f)
                # "polygt": [["x:"], [[206, 251, 386, 542, 620, 646, 550, 358, 189, 140]], ["y:"],
                #            [[633, 811, 931, 946, 926, 976, 1009, 989, 845, 629]], ["PETROSAINS"], ["c"]]
                poly_lst = jfl['polygt']

                n_texts = len(poly_lst)/6
                for i in range(0,len(poly_lst),6):
                    item = {}
                    x_lst = numpy.array(poly_lst[i+1][0])
                    y_lst = numpy.array(poly_lst[i+3][0])
                    np1 = numpy.column_stack((x_lst, y_lst))
                    # print(np1, type(np1))
                    item['poly'] = np1
                    item['text'] = poly_lst[i+4]
                    # print(item)
                    lines.append(item)
