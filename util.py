...
def mask_to_box(targets, outputs, threshold=0.3, is_polygon=False):
    ...
    if is_polygon:
        ...
        boxes, scores = polygons_with_out_contours(outputs[0], segmentation[0], width, height,
                                                   min_size, max_candidates, box_threshold)
    else:
...
def boxes_from_bitmap(output, bitmap, width, height, min_size, max_candidates, box_threshold):
    ...
    # num_contours = min(len(contours[0].shape[0]), max_candidates)
    # num_contours = min(contours[0].shape[0], max_candidates)
    num_contours = 1
...
def polygons_with_out_contours(output, bitmap, width, height, min_size, max_candidates, box_threshold):
    assert bitmap.size(0) == 1
    ...
    for contour in contours[:max_candidates]:
        # contour = numpy.array((bitmap * 255).astype(np.uint8))
